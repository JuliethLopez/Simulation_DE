...
## Set environment
#setwd("/Users/mariaalejandrarojo/Desktop/02_Estadistica_genómica/06_proyecto_SG/Proyecto_SingleCell_Est.Gen") # María
#outdir <- "/Users/mariaalejandrarojo/Desktop/02_Estadistica_genómica/06_proyecto_SG/Proyecto_SingleCell_Est.Gen/plots" # María
setwd("C:/Users/Administrator/Documents/Jaguar/Simulation_DE") # Julieth
outdir <- "C:/Users/Administrator/Documents/Jaguar/Simulation_DE/figures" # Julieth
...
params <- splatEstimate(data_counts) #  parametros para la simulación 
params
...
# Plot the elbow plot
ElbowPlot(object = data_0_PCA, ndims = 40)
...
DimPlot(data_0_cl, group.by = "RNA_snn_res.0.5")   # visualizar PCA despues de cluster
...
plot(density(all_DEGs$avg_log2FC))
...
params_mejorada <- setParams(params_mejorada, dropout.type = "none", dropout.mid = -0.15, dropout.shape = -1.06) # controla los valores extremos

# Ejecutar una nueva simulación con los parámetros mejorados
set.seed(42)

# porporciones celulares para: CD4 T, CD8 T, B, mono, NK, DC
sim.groups <- splatSimulate(
  params_mejorada, 
  group.prob = c(0.45, 0.23, 0.15, 0.10, 0.04, 0.03), # Probabilidades de pertenencia a cada grupo
  de.prob = c(0.10, 0.09, 0.07, 0.06, 0.04, 0.04),    # Probabilidades de expresión diferencial para cada grupo
...
DimPlot(data_1_cl, label = TRUE, reduction = "umap", pt.size = 0.5) + NoLegend()

table(Idents(data_1_cl)) #numero de celulas por cluster
table(colData(sim.groups)$Group)# Verificar la distribución de los grupos simulados

#saveRDS(data_1_cl, file = "simulation_Final_preprocessed.rds")
data_1_cl <- readRDS("simulation_Final_preprocessed.rds")
...
p.markers.lst=vector(mode="list", length=6)
names(p.markers.lst)=paste0("Cluster",1:6)

for(i in 1:6){
  df = as.data.frame(de_fac_groups[[i]])
  plt <- ggplot(df, aes(x=de_fac_groups[[i]])) + 
    geom_density()
  p = ggplotGrob(plt)
  p.markers.lst[[paste0("Cluster",i)]]=p
}

pdf(paste0("density_plots",".pdf"),height = 14, width = 14)
titlePanel=textGrob("Clusters",gp=gpar(fontsize=20,font=3)) 
p.panel=arrangeGrob(grobs=p.markers.lst, ncol=3, top=titlePanel)
grid.arrange(p.panel)
dev.off()

# only one density plot
df = as.data.frame(de_fac_groups[[4]])
colnames(df) = "DEGs_value"
summary(df)
df = subset(df,DEGs_value>1.2)
#nrow(df) # 48 DEGs para el grupo 4 entre 1.2 y 1.5
plt <- ggplot(df, aes(x=DEGs_value)) + 
  geom_density() + 
  geom_vline(xintercept = c(1,1.2,2,3),linetype = 1, color = c(4,rep(2,3))); plt
...
crear_resumen <- function(expected_degs, sim.groups) {
  gene_names <- c()
  clusters <- c()
  defac_groups <- c()
...
# recalcular el p-valor ajustado sobre todos los resultados, ya que FindAllMarkers ajusta los p-valores
# para cada familia (por ejemplo, grupo 0 vs todos los otros, grupo 1 vs todos los otros)
...
# correr con deseq2 paquete original
library("DESeq2")

comparison_result_deseq2_1 <- FindAllMarkers(object = data_1_cl, test.use = "DESeq2")

cts <- data_1_cl@assays$RNA$counts
head(cts)
coldata <- data.frame(Cluster=data_1_cl$Cluster)
head(coldata)
unique(coldata)
...
############# AUROC ######
#curva roc: sensibilidad (TPR) vs (FPR)
#AUC: area bajo la curva, toma valores entre 0 y 1. Entre mayor sea, mejor es el test aplicado.
...
############### PRAUC ######
# cuantifica qué tan bien un modelo puede distinguir entre clases, precisión vs recall
#El area bajo la curva da valores entre 0 y 1, entre mayor sea, mejor es la prueba
...
Summary_metrics <- read.csv("~/Jaguar/Simulation_DE/Summary_metrics.csv", sep=";")
View(Summary_metrics)

# Transformar la matriz en long format
df = melt(Summary_metrics)
colnames(df) = c("Test", "Metric", "Value")
...
df$Value_discrete = as.factor(df$Value_discrete)
...
ggsave(filename = paste0(outdir,"/Heatmap_test_mestric.jpg"), plot = heatmap_test_metrics)


## Guardar las listas de DEGs de cada método en un .csv
# write.csv(markers, file = "markers_wilcox.csv", row.names = TRUE) #save markers