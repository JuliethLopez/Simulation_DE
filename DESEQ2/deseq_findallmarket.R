...
comparison_result_DESeq2SIM <- FindAllMarkers(data_1_cl, test.use = "DESeq2", only.pos = TRUE, logfc.threshold = log2(1.5))

# Crear la tabla de resumen para DESeq2 y categorizar DEGs según niveles de logfc
summary_over_df_DESeq2SIM <- comparison_result_DESeq2SIM %>%
  filter(p_val_adj < 0.05) %>%
  mutate(Gene = rownames(comparison_result_DESeq2SIM)[1:n()]) %>%
