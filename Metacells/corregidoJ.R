...
for (i in seq_along(DEFac_columns)) {
  ...
  # Compute SecondDEFac as weighted average of Group 2 and Group 3
  df_temp <- df %>%
    mutate(FirstDEFac = .[[FirstDEFac_col]],
           SecondDEFac = rowSums(across(all_of(columns_SecondDEFac))) / sum(ncell_SecondDEFac),  # Usamos across para seleccionar columnas
           FoldChange = FirstDEFac / SecondDEFac
    ) %>%
    select(cluster = FirstDEFac, Gene, SecondDEFac, FoldChange)
  head(df_temp)
...
df_final <- do.call(rbind, result_list)
...
## Crear tabla de resumen genes esperados para preparar el calculo de las metricas
expected_degs_levels_df <- bind_rows(expected_degs_levels)
all_genes_df <- bind_rows(all_genes)

head(expected_degs_levels_df)
dim(expected_degs_levels_df) #90000 4
head(all_genes_df)
dim(all_genes_df) #90000  2

#save expected genes
write.csv(expected_degs_levels_df, file = "DEGs_expected.csv", row.names = TRUE)
write.csv(all_genes_df, file = "all_genes.csv", row.names = TRUE)

## Crear una tabla con el número de DEGs esperados por nivel en cada clúster
# real data
deg_counts_filtered <- lapply(expected_degs_levels, function(group){
  group %>%
...
head(DE_metacells_deseq2_groups)
# save results
...
markers_metacells_t_unweighted = DE_classes(DE_metacells_t_unweighted, min_pct_difference = 0.2, max_p_val_adj = 0.05, type="predicted")
markers_metacells_t_weighted = DE_classes(DE_metacells_t_weighted, min_pct_difference = 0.2, max_p_val_adj = 0.05, type="predicted")
markers_metacells_wilcox = DE_classes(DE_wilcox, min_pct_difference = 0.2, max_p_val_adj = 0.05, type="predicted")
markers_metacells_mast = DE_classes(DE_mast, min_pct_difference = 0.2, max_p_val_adj = 0.05, type="predicted")
...
names(wb) = sub(" ","_",names(wb)) # list worksheets
# explore date
